...
with open("content_based_filtering.pkl", 'rb') as file1:  
    content_based = pickle.load(file1)
...
with open("collaborative_filtering.pkl", 'rb') as file2:  
    collaborative = pickle.load(file2)
    ...
    if type == "Content_based":
        ...
        if type1 == "Download file sẵn có":
            ...
            if ma_sp != '' and so_sp_denghi != '':
                ...
                st.code(str(so_sp_denghi)+' sản phẩm đề xuất của mã sản phẩm '+ str(ma_sp) + ' là: '+ str(content_rec))
    else:
        ma_sp = st.selectbox(label = "Chọn mã sản phẩm:", options = option_list)
        so_sp_denghi = st.number_input(label = "Nhập số sản phẩm muốn đề nghị (tu 1-20)", min_value=1, max_value=20)
        if ma_sp != '' and so_sp_denghi != '':
